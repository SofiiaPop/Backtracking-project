...
- **\_\_init\_\_**: Ініціалізує граф, приймаючи параметр `graph`, який є матрицею суміжності. Якщо `graph` не наданий, використовується функція `generate_adjacency_matrix` для створення випадкового графа. Також ініціалізується атрибут `verticles`, що визначає кількість вершин у графі.
...
- **generate_adjacency_matrix**: Генерує випадкову матрицю суміжності, що представляє граф. Вона визначає випадкову кількість вершин і ребер, а потім випадковим чином встановлює з'єднання між вершинами, формуючи неорієнтований граф.

### 3. Головна функція `main`

- **main**: Головна функція програми, яка створює екземпляр класу `Graph`, надає користувачеві можливість ввести кількість кольорів, яку він хоче використовувати для розфарбування графа, та виводить розв'язок або повідомлення про те, що розв'язок не існує. Якщо розв'язок існує, викликається функція для візуалізації графа.

Звіт буде розширений з врахуванням вказаних пунктів.

## Вхідні дані
- Генерується випадкова матриця суміжності для графа
- Користувач вводить кількість кольорів для розфарбування
...
- У найгіршому випадку, коли алгоритм повинен розглянути всі можливі комбінації кольорів для кожної вершини, складність становить 𝑂(𝑚^𝑁), де N - кількість вершин у графі, а m - кількість кольорів. Рекурсивний стек функції розфарбовування графіка потребує O(V) простору.
...
![alt text](image_2024-05-13_11-59-23.png)
